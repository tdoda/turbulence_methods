...
if ~isempty(AA_piezo) && make_figures && (plot_kinematics || plot_rawaccel)
    ...
    if MM == 2
        sp(1) = subplot(1,MM,1);
        plot([AA_raw filtfilt(b, a, AA_raw)], Indep_var_fast);grid on
    ...
    sp(MM) = subplot(1,MM,MM);
    plot([AA filtfilt(b, a, AA)], Indep_var_fast,'linewidth',2);grid on
...
if shear_count >0  && make_figures && plot_dissipation
    ...
    set(gca,'ylim',y_limits)
%     set(gca,'ydir','rev')
    set(gca,'view',[0,-90]) % For R2018b and newer, this prevents labels from overlapping axes
