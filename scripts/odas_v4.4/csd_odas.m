...
if (isempty(Window)) % Generate cosine window
    ...
    Window=Window*0.5;    % EPFL change: mutiplied by 00.5 according to the definition of Hann window (superfluous since then it is normalized, but more precise). SEB
% %     Window = Window ./ (sqrt(mean(Window.^2))); ... % EPFL_change:
% moved below to allow use other windows. SEB
        % Force it to have a mean-square of 1.
end
Window = Window ./ (sqrt(mean(Window.^2)));  % EPFL_change: SEB
if isvector(Window)
