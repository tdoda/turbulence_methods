...
function [param] = load_parameters_Geneva(lakename,date,general_data_folder,direction,instrument)
...
elseif strcmp(lakename,"default")
    ...
    if nargin<5 % Instrument was not specified
        ...
        param.SNname='310';
        warning('Instrument was not specified: use microCTD SN310 as default.')
    else
        param.instrument=instrument;
        if strcmp(param.instrument,'microCTD')
            param.SNname='310';
        else
            param.SNname='028'; % VMP
        end
    end
...
param.info.pmax = 100; % Maximum depth [m] of the bin-profiles (not used to detect the profiles, only velocity criterion)
param.info.dp = 0.5; % Bin overlap [m]
param.info.dpD = 1; % Bin size [m]

%% Campaign-specific values
%**************************************************************************
if strcmp(date,'20190826') % 24h experiment
    param.folder = [general_data_folder,'20190826\Level0\'];
    %param.filename_list={'VMP_005','VMP_009','VMP_012','VMP_017','VMP_018','VMP_020','VMP_023'}; % Several files can be listed here
    param.filename_list={'VMP_009','VMP_018','VMP_023'};
