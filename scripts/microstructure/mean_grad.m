...
%MEAN_GRAD Computes the average vertical gradient of x for each bin.
%
%   INPUTS:
...
%
%   OUTPUTS:
%   grX (1*nbin double array): gradient of x in each bin
% 
...
for i=1:nn
    ...
    if sum(x(jj),'omitnan')>10 % At least 10 values are not NaN (T. Doda)
        ...
        [grX(i),sgrX(i)] = slope(-x0,y0); % Returns slope from linear fit [x/dbar] and std of the slope
